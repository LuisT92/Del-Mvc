...
public class GestorCompras {
    ...
    ArrayList<ItemPOCModelo> miModeloPOC;
    ArrayList<ItemDelModelo> miModeloDel;
    ...
    public GestorCompras() throws IOException {
        ...
        try {
			...
			miReporteNS = mapper.readValue(new FileInputStream("C:\\Users\\LENOVO\\Desktop\\post-api-mvc-rest\\arregloItemsV1.json"), ListaReportesNivelStock.class );
    ...
    public ArrayList<ItemPOCModelo> getPOC() {
        ...
        miModeloPOC = new ArrayList<ItemPOCModelo>();

                for(int j=0; j<miPeticionOC.getItems().size(); j++)
                {
                    //ItemPOCModelo(int cantidad, double valorUnitario, double subtotal, double total)
                    ...
                    miModeloPOC.add(nodo);
                }

        return miModeloPOC;
    }
    public ArrayList<ItemDelModelo> getDel() {

        miModeloDel = new ArrayList<ItemDelModelo>();

        for(int j=0; j<miModeloDel.size(); j++)
        {
            //ItemDelModelo(int cantidad, double valorUnitario, double subtotal, double total)
            ItemDelModelo nodo = new ItemDelModelo(
                    miModeloDel.get(j).getVendedor()
                    );
            miModeloPOC.add(nodo);
        }
        if(nodo.getVendedor() == 100){
            //eliminar valor
            miModeloDel.remove(j);
        }
